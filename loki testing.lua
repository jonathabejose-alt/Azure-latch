...
local function Skill2(char)
    ...
    local machSound = Instance.new("Sound")
    machSound.SoundId = "rbxassetid://129129031789518"
    machSound.Volume = 1.5
    machSound.Parent = root
    machSound:Play()
    Debris:AddItem(machSound, 2)
    
    local grabbed = false
    local timeout = 0
    local maxTimeout = 200
    
    while not grabbed and timeout < maxTimeout and TooSlowActive do
        local currentBall = Workspace.Terrain:FindFirstChild("Ball")
        if not currentBall then
            break
        end
        
        local direction = (currentBall.Position - root.Position).Unit
        local horizontalDirection = Vector3.new(direction.X, 0, direction.Z).Unit
        
        if horizontalDirection.Magnitude > 0 then
            root.CFrame = CFrame.new(root.Position, Vector3.new(currentBall.Position.X, root.Position.Y, currentBall.Position.Z))
            root.AssemblyLinearVelocity = horizontalDirection * 80
        else
        ...
        rayParams.FilterType = Enum.RaycastFilterType.Blacklist
        local rayResult = workspace:Raycast(root.Position + Vector3.new(0, 10, 0), Vector3.new(0, -100, 0), rayParams)
        if rayResult then
            root.Position = Vector3.new(root.Position.X, rayResult.Position.Y + 3, root.Position.Z)
        end
    ...
    if grabbed then
        ...
        catchAnim.AnimationId = "rbxassetid://82371642989185"
        local catchTrack = animator:LoadAnimation(catchAnim)
        catchTrack.Priority = Enum.AnimationPriority.Action4
        catchTrack:Play()
